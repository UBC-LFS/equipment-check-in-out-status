...
function LoanTables ({ loans }) {
  ...
  const [queryLoanList, updateQueryLoanList] = useState(loans.equipmentList)

  function filterWithQuery (arr, property, value, dateParameter, fromDate, toDate) {
    if (!property || !value) {
      return arr.filter(el => {
        if (el[dateParameter]) {
          if (fromDate && toDate) { return Date.parse(el[dateParameter]) >= fromDate && Date.parse(el[dateParameter]) <= toDate } else if (fromDate) { return Date.parse(el[dateParameter]) >= fromDate } else if (toDate) { return Date.parse(el[dateParameter]) <= toDate } else { return true }
        }
    ...
    <div className='main-container'>
      ...
      <section className='table-section'>
        <div className='table-container'>
          ...
          <Table striped hover id='signed-out' className='loan-table'>
            <thead>
              <tr>
                <th>Type</th>
                <th>Description</th>
        ...
        <div className='table-container'>
          ...
          <Table striped hover id='returned' className='loan-table'>
            <thead>
              <tr>
                <th>Type</th>
                <th>Description</th>
